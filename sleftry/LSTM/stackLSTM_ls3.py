...
data=df.iloc[:,4:53].values  #取第3-10列
data[:1]

def get_train_data(batch_size=60,time_step=20,train_begin=1,train_end=3000):
    batch_index=[]
    ...
    for i in range(len(normalized_train_data)-time_step):
       ...
       x=normalized_train_data[i:i+time_step,1:49]
...
def get_test_data(time_step=20,test_begin=3001):
    ...
    for i in range(size-1):
       x=normalized_test_data[i*time_step:(i+1)*time_step,1:49]
       y=normalized_test_data[i*time_step:(i+1)*time_step,0]
       test_x.append(x.tolist())
       test_y.extend(y)
    test_x.append((normalized_test_data[(i+1)*time_step:,1:49]).tolist())
