# -*- coding: utf-8 -*-
"""
Created on Thu Jan 17 14:44:31 2019
...
lr=0.0006         #學習率
#——————————匯入資料———---------
f=open('ml_data_all.csv')
df=pd.read_csv(f)     #讀入股票資料
data=df.iloc[:,4:9].values  #取第3-10列
data[:1]
...
def get_train_data(batch_size=60,time_step=20,train_begin=0,train_end=3000):
    ...
    for i in range(len(normalized_train_data)-time_step):
       ...
       x=normalized_train_data[i:i+time_step,5:8]
...
def get_test_data(time_step=20,test_begin=3001):
    data_test=data[test_begin:5186]
    ...
    for i in range(size-1):
       x=normalized_test_data[i*time_step:(i+1)*time_step,5:8]
       y=normalized_test_data[i*time_step:(i+1)*time_step,4]
       test_x.append(x.tolist())
       test_y.extend(y)
    test_x.append((normalized_test_data[(i+1)*time_step:,5:8]).tolist())
...
def prediction(time_step=20):
    ...
    with tf.Session() as sess:
        ...
        save_path = saver.save(sess, r"D:/TF TEST LEARNING/LSTM/model_save2/")
        print(save_path)
        #引數恢復
        module_file = tf.train.latest_checkpoint('D:\TF TEST LEARNING\LSTM\model_save2')
        saver.restore(sess, module_file)
        #saver.restore(sess, r"D:\TF TEST LEARNING\LSTM\model_save2") 
        test_predict=[]
