...
def get_train_data(batch_size,time_step,train_end):
    ...
    for i in range(len(normalize_train_data)-time_step):
        ...
        x=normalize_train_data[i:i+time_step,1:all_num]
        #add dimension
...
def get_test_data(time_step,test_begain):
    ...
    sample_size=(len(normalize_test_data)-time_step)//time_step
    test_x,test_y=[],[]
...
def test_lstm(time_step,train_begain,learning_rate):
    ...
    with tf.variable_scope("layers_lstm",reuse=tf.AUTO_REUSE):
        predict_label, _ =rnn_lstm(features)
    saver=tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        #restore parameter
        module_file=tf.train.latest_checkpoint('D:\TF TEST LEARNING\LSTM\model')
        saver.restore(sess,module_file)
        test_predict=[]
        for step in range(len(test_x)-1):
            prob=sess.run(predict_label,feed_dict={features:[test_x[step]],keep_prob:1})
